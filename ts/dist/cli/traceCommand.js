...
exports.traceLast = traceLast;
exports.traceVerify = traceVerify;
const store_1 = require("../storage/store");
const config_1 = require("../config");
function makeStore() {
    const config = (0, config_1.getConfig)();
    const store = new store_1.ObservationStore(config.dbPath);
    store.createTables();
    return store;
}
...
function compactText(node, indent = 0) {
    ...
    if ("operation" in node.span) {
        ...
        if (span.inputTokens > 0 || span.outputTokens > 0) {
            lines.push(`${prefix}  tokens: ${span.inputTokens} in / ${span.outputTokens} out`);
        }
    }
    else {
 ...
 */
function traceList(limit = 10, errorsOnly = false) {
    ...
    try {
        ...
        const header = `${"TRACE_ID".padEnd(34)}` +
            `${"ROOT SPAN".padEnd(25)}` +
            `${"STARTED".padEnd(20)}` +
            `${"SPANS".padStart(6)}` +
            `${"ERRORS".padStart(7)}`;
        ...
        for (const t of traces) {
            const row = `${String(t["traceId"]).padEnd(34)}` +
 ...
 */
function traceShow(traceId, verbose = false, asJson = false) {
    const store = makeStore();
    try {
        // Support prefix matching
        const traces = store.listTraces(500);
        const matched = traces.filter((t) => String(t["traceId"]).startsWith(traceId));
        if (matched.length === 0) {
        ...
        if (matched.length > 1) {
            const ids = matched
                ...
                .join("\n  ");
            console.log(`Error: Multiple traces match '${traceId}'. Be more specific:\n  ${ids}`);
            return 1;
        }
 ...
 */
function traceLast(asJson = false) {
    const store = makeStore();
    try {
...
function traceVerify() {
    ...
    try {
        const traces = store.listTraces(1);
        if (traces.length === 0) {
            console.log("ERROR: No traces found. Run the app first to produce a trace.");
            return 1;
        }
        const traceId = String(traces[0]["traceId"]);
        ...
        if (rootIsLlm) {
            issues.push("Root span is an LLM call, not an observe-wrapped function. " +
        ...
        if (!rootIsLlm) {
            ...
            const hasInput = rootObserve.input != null;
            const hasOutput = rootObserve.output != null;
            if (!hasInput) {
                issues.push("Root span input is null. The observe-wrapped function's " +
            ...
            if (!hasOutput) {
                issues.push("Root span output is null. The observe-wrapped function's " +
        ...
        if (llmCount === 0) {
            issues.push("No LLM child spans found. Ensure enableStorage() is called " +
