...
datasetCmd
    ...
    .option("--select <mode>", "How to select the span (root, last_llm_call, by_name)", "root")
    ...
    if (options.expectedOutput) {
        ...
        if (!raw) {
            console.error("--expected-output flag set but no JSON provided on stdin.");
            process.exit(1);
        }
    ...
    try {
        ...
        console.log(`Saved to dataset '${dataset.name}' — now ${dataset.items.length} item(s).`);
    }
    catch (exc) {
        console.error(`Error: ${exc.message}`);
        process.exit(1);
...
datasetCmd
    ...
    const { discoverDatasetFiles, validateDatasetFile, } = require("../evals/datasetRunner");
    const target = searchPath ?? getConfig().datasetDir;
    const datasetFiles = discoverDatasetFiles(target);
    ...
    for (const datasetFile of datasetFiles) {
        const errors = validateDatasetFile(datasetFile);
        if (errors.length > 0) {
            filesWithErrors++;
