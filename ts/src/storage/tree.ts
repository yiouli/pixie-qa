...
export class ObservationNode {
  ...
  private _observeToText(prefix: string, indent: number): string {
    const span = this.span as ObserveSpan;
    const lines: string[] = [];
  ...
  private _llmToText(prefix: string, indent: number): string {
    const span = this.span as LLMSpan;
    const lines: string[] = [];
    lines.push(
...
function formatMessage(
  ...
  if (msg.role === "system") {
    return `system: ${(msg as SystemMessage).content}`;
  }
  if (msg.role === "user") {
    const parts = (msg as UserMessage).content
...
export function buildTree(
  spans: ReadonlyArray<ObserveSpan | LLMSpan>
): ObservationNode[] {
  ...
  for (const node of nodes.values()) {
    node.children.sort(
      (a, b) => a.span.startedAt.getTime() - b.span.startedAt.getTime()
    );
  }
