...
# Initialize pixie instrumentation (idempotent — safe to call multiple times)
px.init()
...
def answer_question(question: str) -> str:
    ...
    with px.start_observation(input=question, name="rag-chatbot") as observation:
        context_chunks = retrieve_docs(question)
        context = "\n".join(context_chunks)

        # Record the retrieved context as metadata on the observation
        observation.set_metadata("retrieved_chunks", context_chunks)
        observation.set_metadata("context", context)
